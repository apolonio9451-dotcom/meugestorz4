...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 1. Fetch chatbot settings for this company
    ...
    // 3. Check if sender is a known client
...
- Se não souber algo, diga que vai verificar e retornar.`;
    ...
    let replyText = aiData.choices?.[0]?.message?.content || "Desculpe, não consegui processar sua mensagem.";

    // 6. Check if AI wants to send media
    ...
    if (mediaMatch && mediaFiles) {
      ...
      // Remove the media tag from the reply
      replyText = replyText.replace(/^\[ENVIAR_MEDIA:.+?\]\s*/i, "").trim();
    }

    // 7. Simulate presence and send
    if (mediaToSend) {
      // Simulate recording for audio files
      const presenceType = mediaToSend.file_type === "audio" ? "recording" : "composing";
      const presenceDuration = mediaToSend.file_type === "audio"
        ? 5000 + Math.random() * 3000  // 5-8s for audio
        : 3000 + Math.random() * 2000; // 3-5s for video

      await simulatePresence(apiUrl, apiToken, phone, presenceType, presenceDuration);

      // Send text first if there is one
      if (replyText) {
        await simulatePresence(apiUrl, apiToken, phone, "composing", 2000 + Math.random() * 2000);
      ...
      // Text-only response
      const typingDuration = 3000 + Math.random() * 2000; // 3-5s
    ...
    await supabase.from("chatbot_logs").insert({
      ...
      message_sent: replyText.slice(0, 500),
