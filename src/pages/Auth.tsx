...
import { Mail, Lock, User, FlaskConical, Clock, Phone, Info } from "lucide-react";
import appLogo from "@/assets/logo.png";
import { differenceInHours, parseISO } from "date-fns";

/** WhatsApp do proprietário do sistema — exibido para quem criar conta sem link de indicação */
const OWNER_SUPPORT_WHATSAPP = "81985380657";
...
export default function Auth() {
  ...
  const [ownerWhatsapp, setOwnerWhatsapp] = useState<string>(OWNER_SUPPORT_WHATSAPP);
  ...
  const hasTrialLink = !!trialToken && !!trialInfo;
  ...
  /** Registro com link de indicação (trial link) */
  const handleRegisterWithTrial = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!trialToken || !trialInfo) return;
  ...
  /** Registro direto (sem link de indicação) — entra em período de teste de 7 dias */
  const handleRegisterDirect = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const password = form.get("password") as string;
    const fullName = form.get("fullName") as string;
    const companyName = (form.get("companyName") as string || "").trim() || "Minha Empresa";
    const whatsapp = (form.get("whatsapp") as string || "").trim();
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          whatsapp,
          is_trial: false, // handle_new_user will create a fresh company + owner role
        },
        emailRedirectTo: window.location.origin,
      },
    });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-md">
        ...
        {/* Trial Banner (only with trial link) */}
        {hasTrialLink && (
        ...
        <Tabs defaultValue={hasTrialLink ? "register" : "login"}>
          ...
          <TabsContent value="register">
            <div className="rounded-2xl border border-border bg-card p-6 space-y-5">
              {hasTrialLink ? (
                /* ── Com link de indicação ── */
                <form onSubmit={handleRegisterWithTrial} className="space-y-4">
                  <div className="space-y-2">
                  ...
                  {resendEmail && (
                    <Button type="button" variant="outline" onClick={handleResendConfirmation}
                      disabled={resendingConfirmation} className="w-full h-11 rounded-xl">
                      {resendingConfirmation ? "Reenviando..." : "Reenviar email de verificação"}
                    </Button>
                  )}
                </form>
              ) : (
                /* ── Sem link de indicação — cadastro direto com período de teste ── */
                <form onSubmit={handleRegisterDirect} className="space-y-4">
                  ...
                  <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 flex items-start gap-2">
                    ...
                    <div className="text-xs text-muted-foreground">
                      <p className="font-semibold text-foreground text-sm mb-1">Período de Teste — 7 dias grátis</p>
                      <p>Crie sua conta e explore o sistema. Após o período de teste, entre em contato para ativar seu plano.</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input name="companyName" placeholder="Minha Empresa"
                        className="pl-10 bg-input border-border text-foreground placeholder:text-muted-foreground h-12 rounded-xl" />
                    </div>
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="rounded-lg border border-border bg-muted/30 p-3 text-center space-y-2">
                    <p className="text-xs text-muted-foreground">Dúvidas? Fale com o administrador:</p>
                    <a
                      href={`https://wa.me/${ownerWhatsapp.replace(/\D/g, "")}?text=${encodeURIComponent("Olá! Criei minha conta no Meu Gestor e gostaria de mais informações.")}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <Button type="button" variant="outline" className="w-full h-10 rounded-xl gap-2 text-green-600 border-green-600/30 hover:bg-green-600/10">
